...
function DashboardPage() {
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#f7f4ee_0%,#eff5ff_55%,#f8fbff_100%)] text-slate-900">
      <div className="mx-auto max-w-7xl px-6 py-8 sm:px-8 lg:px-10">
        <header className="mb-8 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <p className="mb-3 inline-flex rounded-full bg-[var(--color-sky)] px-4 py-2 text-xs font-bold uppercase tracking-[0.22em] text-[var(--color-ocean)]">
              Ledgerly Admin
            </p>
            <h1 className="font-display text-4xl font-black tracking-tight text-slate-950 sm:text-5xl">
              Dashboard
            </h1>
            ...
            <p className="mt-3 text-sm text-slate-500">
              Signed in as{" "}
              <span className="font-semibold text-slate-700">Jimmy Lawson</span>
            </p>
          </div>
          <div className="rounded-[26px] border border-slate-200/80 bg-white/85 px-5 py-4 shadow-[0_24px_80px_-48px_rgba(16,35,63,0.45)] backdrop-blur">
            ...
            <Card className="border-white/70 bg-white/90 shadow-[0_24px_80px_-48px_rgba(16,35,63,0.45)] backdrop-blur">
              ...
              <CardContent className="space-y-3">
                {recentContributions.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50/75 px-4 py-3"
                  >
                    <div>
